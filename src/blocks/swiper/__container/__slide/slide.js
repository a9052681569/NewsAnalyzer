  ...
  // верстаем карточку
    create() {
      ...
      <div class="slide__about">
          <div class="slide__about-img"></div>
          <h3 class="slide__about-title h3-text"></h3>
          <p class="slide__about-subtitle card-text"></p>
      </div>
      <p class="slide__text card-text"></p>`);
      // наполняем карточку внешним контентом
      slide.querySelector(".slide__about-img").style.backgroundImage = `url(${this.options.link})`;
      slide.querySelector('.slide__date').textContent = this.options.date;
      slide.querySelector(".slide__about-title").textContent = this.options.title;
      slide.querySelector(".slide__about-subtitle").textContent = this.options.subtitle;
