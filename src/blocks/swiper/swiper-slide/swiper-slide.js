  ...
  // верстаем карточку
    create() {
      ...
      <div class="swiper-slide__about">
          <div class="swiper-slide__about-img"></div>
          <h3 class="swiper-slide__about-title h3-text"></h3>
          <p class="swiper-slide__about-subtitle card-text"></p>
      </div>
      <p class="swiper-slide__text card-text"></p>`);
      // наполняем карточку внешним контентом
      slide.querySelector(".swiper-slide__about-img").style.backgroundImage = `url(${this.options.link})`;
      slide.querySelector('.swiper-slide__date').textContent = this.options.date;
      slide.querySelector(".swiper-slide__about-title").textContent = this.options.title;
      slide.querySelector(".swiper-slide__about-subtitle").textContent = this.options.subtitle;
